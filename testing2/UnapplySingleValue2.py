...
class JHashMapWrapper(Generic[K,V]):
...
class JHashSetWrapper(Generic[K]):
...
# Works:
result_map = unapply_map(map).jmap
result_set = unapply_set(set).jset

# This fails to compile because there are _two_ possible returned values.
result = [unapply_map(x).jmap if isinstance(x, dict) else unapply_set(x).jset for x in [map, set]]